...
beforeEach(async() => {
    ...
    usersContract = await new web3.eth.Contract(JSON.parse(interface)).deploy({ data: bytecode }).send({ from: accounts[0], gas: "1000000" });
});
...
describe("The UsersContract", async() => {
    ...
    it("should join a user", async() => {
        let name = "Ivan";
        let lastname = "Cano";
        await usersContract.methods.join(name, lastname).send({ from: accounts[0], gas: "500000" });
    });
    ...
    it("should retrieve a user", async() => {
        ...
        assert.equal(lastname, user[1]);
    });

    it("should not allow joining an account twice", async() => {
        await usersContract.methods.join("Pedro", "Lopez").send({ from: accounts[0], gas: "500000" });
        try {
            await usersContract.methods.join("Ana", "Gomez").send({ from: accounts[0], gas: "500000" });
            assert.fail("same account cannot join twice");
        } catch(e) {
